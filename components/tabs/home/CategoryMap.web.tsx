...
export default function CategoryMapWeb({ items, loading, kind }: Props) {
  ...
  return <View style={styles.container}>
    ...
    {loading ? <View style={styles.badge}><ActivityIndicator size="small" color={theme.COLORS.primary} /><Text>Updating {kind}s</Text></View> : !mapped.length ? <View style={styles.badge}><Text>No {kind}s with map locations found.</Text></View> : null}
...
const styles = StyleSheet.create({ container: { flex: 1, minHeight: 560 }, state: { flex: 1, minHeight: 420, alignItems: "center", justifyContent: "center", padding: 24, gap: 8 }, title: { color: theme.COLORS.textPrimary, fontWeight: "700" }, text: { color: theme.COLORS.textSecondary, textAlign: "center" }, retry: { color: theme.COLORS.primary, fontWeight: "700", marginTop: 8 }, badge: { position: "absolute", top: 16, alignSelf: "center", flexDirection: "row", gap: 8, padding: 10, borderRadius: 12, backgroundColor: "#ffffff", color: theme.COLORS.textSecondary }, preview: { position: "absolute", left: 16, right: 16, bottom: 16, padding: 16, borderRadius: 16, backgroundColor: "#ffffff", shadowColor: "#000", shadowOpacity: 0.15, shadowRadius: 12 }, previewTitle: { color: theme.COLORS.textPrimary, fontSize: 16, fontWeight: "800" }, button: { marginTop: 10, padding: 10, borderRadius: 10, backgroundColor: theme.COLORS.primary, alignItems: "center" }, buttonText: { color: "#fff", fontWeight: "700" }, });
